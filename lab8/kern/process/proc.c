...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    */
	...
	if((ret=copy_mm(clone_flags,proc))!=0) goto bad_fork_cleanup_kstack;
	if((ret=copy_files(clone_flags,proc))!=0) goto bad_fork_cleanup_fs;
...
load_icode(int fd, int argc, char **kargv) {
	...
	if((ret=load_icode_read(fd,elf,sizeof(struct elfhdr),0))!=0) goto bad_pgdir_cleanup_mm;
    ...
    uint32_t vm_flags, perm, ph_i;
    for (ph_i=0; ph_i < elf->e_phnum; ph_i ++) {
		if((ret=load_icode_read(fd,ph,sizeof(struct proghdr),elf->e_phoff+ph_i*sizeof(struct proghdr)))!=0) goto bad_pgdir_cleanup_mm;
        ...
        while (start < end) {
			...
			if((ret=load_icode_read(fd,page2kva(page) + off,size,from))!=0) goto bad_pgdir_cleanup_mm;
	...
	//(new 6)setup uargc and uargv in user stacks (to do myself......)
